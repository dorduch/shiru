...
- **Batch 6 — keypad + hybrid-dark (done):** `PinGateScreen`, `ChangePinScreen`, `GuidedCaptureScreen`. Built `LanternKeypad` — but only the keypad/dots/lockout-overlay were a true near-duplicate; the two screens' step semantics, success paths, and back-nav direction genuinely differ and stayed separate, not merged. `GuidedCaptureScreen`'s manual dark-paint hack (hardcoded `night1`/`cream` regardless of theme) dropped in favor of reading the ambient theme like every other screen. Along the way, found and fixed a real pre-existing bug in `pin_gate_screen_test.dart` (missing `theme:` in its test harness meant it null-checked at setup, contributing 4 tests to the "known failures" baseline for reasons unrelated to flakiness) — full suite went from 136/9 to 140/5.
- **Batch 7 — heaviest (done — rollout complete):** `StorytimeLaunchScreen`, `StScenePlayer`, `StoryPlayerScreen`. The read-along sync engine (positionStream/playerStateStream StreamBuilders, highlightedWordIndex computation, seek/play-pause) sits inline inside `StoryPlayerScreen.build()` interleaved with chrome — the "only touch build()" heuristic from Batches 5-6 didn't hold here, the do-not-touch boundary had to be drawn at the statement level. Zero test coverage existed at this layer; added a permanent `st_scene_player_test.dart` locking the highlight-color invariant, and verified live on a simulator (via the local Firebase emulator, not a real account) against a curated story — highlight tracked playback in real time, pause froze it, seek jumped it correctly, and natural completion navigated to StoryEndScreen. This on-device check was treated as a hard gate, not optional, since nothing in the toolchain (analyze, tests, diff review) can otherwise catch a desynced read-along.

**Rollout status: all 7 batches + Foundation complete**, plus a post-rollout audit (2026-07-11) that found and fixed 3 real regressions in already-"done" batches: `StorytimePrivacyScreen`'s `StToggle` (built `LanternToggle` — `StToggle`'s off-track color was locked, never mode-aware), a systemic `StButton`(ember) vs `GlowButton` accent-color mismatch across 9 call sites in 8 screens (the old `ctaGradient` is muted terracotta, the new one is bright amber — both "safe"/legible per §5 item 4, but visibly inconsistent, which the "safe" judgment never checked for), and `GuidedCaptureScreen`'s `StPrompt`/`StVoiceWave`/`StRecordButton`/`StDots` (migrated in place, confirmed single live consumer). Every screen reachable from a live route now renders on the single permanent Lantern night theme with a consistent accent color. Two items remain genuinely out of scope by design, not oversights: `StoryWizardScreen`/`StoryReviewScreen` and an orphaned unwired `FamilyVoicesTeaserScreen` are dead code (unreachable — `/make/:step` and `/review` unconditionally redirect to `/compose`); the 5 "v1" screens (`KidHomeScreen` et al., §7) are unreachable, substantial CRUD code whose deletion needs an explicit product decision, not a token migration.
