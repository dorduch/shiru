...
|---|---|---|
| `StButton` | `GlowButton` (primary/ember variant) + New: `LanternOutlineButton` | **Both exist** (resolved during Batch 1 — see §5, open question 1). `GlowButton` stays single-purpose (primary CTA only, by design — no variant enum). `LanternOutlineButton` is the quiet secondary button (transparent fill, `hush` border, `moonDim` text) — use it for every non-CTA button instead of `StButton`'s `ghost`/`soft` (hardcode a white fill — wrong on dark) or `line` (renders correctly via mode-aware `textPrimary`, but is a style mismatch, not just a technical gap). |
| `StChoiceCard` | `VoiceCard` / `StorySlot` (context-specific) | **Exists**, but both are purpose-built for the Composer's exact shapes. A general-purpose "selectable card" for other screens (e.g. `ChildSetupScreen`'s avatar picker) doesn't exist yet — new. |
...
- **Batch 2 — simple forms:** `ChildSetupScreen`, `StorytimeAuthScreen`, `AddAudioCaptureScreen`, `AddAudioDetailsScreen`, `VoiceUploadScreen` (needs `LanternTextField`, `LanternSegment`).
- **Batch 3 — lists with state:** `FamilyVoicesScreen`, `VoiceConsentScreen` (needs `LanternChip`).
- **Batch 4 — adaptive/content-heavy:** `StorytimeHomeScreen`, `StoryLibraryScreen` (needs `LanternActionTile`, grid tiles).
- **Batch 5 — bedtime-state-machine screens:** `StoryGeneratingScreen`, `AgeGateScreen`.
