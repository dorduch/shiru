...
- ✅ **REAL clone + narration VERIFIED (2026-06-27):** with the user's upgraded ElevenLabs key in `functions/.secret.local` and the mock removed, `functions/dev/real_clone_demo.mjs` uploaded a genuine voice sample → `processVoiceClone` performed a **real ElevenLabs instant clone** (`status: ready`, real `providerVoiceId`) → synthesized a bedtime line **in the cloned voice** (played back, confirmed by ear). Test voice was then deleted from the account via the same `DELETE /v1/voices/{id}` path `deleteVoice` uses (confirmed gone). Full M4 backend now proven against the live provider, not just a mock.
- ✅ **Client↔emulator round-trip VERIFIED on iOS sim (2026-06-27):** app run with `--dart-define=USE_EMULATOR=true`; reached the live Family Voices screen (s20) which streamed `users/{uid}/voices` from the emulator and rendered the empty state. App boots/onboards/gates clean against emulators — **no API errors** on the sim (the earlier Android errors were platform infra: Play-Services `DEVELOPER_ERROR`, App Check, Firestore gRPC over WiFi — not app defects). NOT done in-app: the consent→capture→clone UI walk (sim file-picker/mic friction; backend already proven above). Fixed: stale "Coming soon" subtitle on the dashboard tile.
